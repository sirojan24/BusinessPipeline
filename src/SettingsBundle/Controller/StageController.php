    ...
    public function savestageAction (Request $request){
        ...
        if($token && $request->getMethod() == 'POST' && ($token->getRole() == 'Admin')){
             ...
             try{  
                ...
                $em->flush();
                $stages = $repository1->findBy(array('companyname' => $user->getCompanyname()));
                 return $this->render('SettingsBundle:Default:manageStages.html.twig', array('name' => $token->getUsername(), 'role' => $token->getRole(),'fullname'=>$fullname,'successmsg' => "Well done ! You successfully add a Stage ",'stages'=> $stages ));
        ...
        if($token){
              $em = $this->getDoctrine()->getManager();
               ...
               try{
                    ...
                    $fullname = $user->getFirstname()." ".$user->getLastname();
                    $stages = $repository1->findBy(array('companyname' => $user->getCompanyname()));
                    return $this->render('SettingsBundle:Default:manageStages.html.twig', array('name' => $token->getUsername(), 'role' => $token->getRole(),'stages'=> $stages,'fullname'=>$fullname,'successmsg' => "Well done ! You successfully delete the stage" ));
        ...
        if($token){
              $em = $this->getDoctrine()->getManager();
               ...
               try{
                    ...
                    $fullname = $user->getFirstname()." ".$user->getLastname();
                    $stages = $repository1->findBy(array('companyname' => $user->getCompanyname()));
                    return $this->render('SettingsBundle:Default:manageStages.html.twig', array('name' => $token->getUsername(), 'role' => $token->getRole(),'stages'=> $stages,'fullname'=>$fullname,'successmsg' => "Well done ! You successfully activate the stage" ));
